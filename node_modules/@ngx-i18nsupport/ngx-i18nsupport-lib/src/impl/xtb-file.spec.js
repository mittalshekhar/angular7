...
describe('ngx-i18nsupport-lib xtb test spec', () => {
    ...
    function readFile(path, masterPath) {
        ...
        if (masterPath) {
            ...
            return index_1.TranslationMessagesFileFactory.fromFileContent('xtb', content, path, ENCODING, optionalMaster);
        }
        else {
            return index_1.TranslationMessagesFileFactory.fromFileContent('xtb', content, path, ENCODING, null);
        }
    }
    /**
    ...
    function readUnknownFormatFile(path, masterPath) {
        ...
        if (masterPath) {
            const masterContent = fs.readFileSync(masterPath, ENCODING);
            const optionalMaster = { xmlContent: masterContent, path: masterPath, encoding: ENCODING };
            return index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(content, path, ENCODING, optionalMaster);
        }
        else {
            return index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(content, path, ENCODING, null);
        }
    }
    describe('xtb format tests', () => {
        ...
        it('should read xtb file', () => {
            const file = readFile(TRANSLATION_EN_XTB);
            expect(file).toBeTruthy();
            expect(file.fileType()).toBe('XTB');
            const tu = file.transUnitWithId(ID_MY_FIRST);
        ...
        it('should emit warnings, if master does not fit', () => {
            const file = readUnknownFormatFile(TRANSLATION_EN_XTB, MASTER_1_XMB);
        ...
        it('should return source language', () => {
            ...
            const file2 = readFile(TRANSLATION_EN_XTB, MASTER_DE_XMB);
            expect(file2.sourceLanguage()).toBe('de');
        });
        it('should return target language', () => {
            const file = readFile(TRANSLATION_EN_XTB, MASTER_DE_XMB);
            expect(file.targetLanguage()).toBe('en');
        });
        it('should loop over all trans units', () => {
            const translatedFile = readFile(TRANSLATION_EN_XTB, MASTER_DE_XMB);
            let count = 0;
        ...
        it('should return source reference with more than 1 linenumber', () => {
            ...
            const file = readFile(TRANSLATION_EN_XTB, MASTER_DE_XMB);
            const tu = file.transUnitWithId(ID_WITH_LINEBREAK);
            expect(tu).toBeTruthy();
            expect(tu.sourceReferences().length).toBe(1);
            expect(tu.sourceReferences()[0].sourcefile).toBe('S:/experimente/sampleapp41/src/app/app.component.ts');
        ...
        it('should not change source reference, because it is not supported in xtb', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null, masterContent);
        ...
        it('should normalize embedded html tags', () => {
            const file = readFile(TRANSLATION_EN_XTB, MASTER_DE_XMB);
            const tu = file.transUnitWithId(ID_WITH_TAG_STRANGE);
        ...
        it('should copy a transunit from file a to file b', () => {
            const file = readUnknownFormatFile(MASTER_1_XMB);
            const tu = file.transUnitWithId(ID_TO_MERGE);
            expect(tu).toBeTruthy();
            const targetFile = readFile(TRANSLATION_EN_XTB, MASTER_1_XMB);
        ...
        it('should not copy source to target for non default lang if not wanted', () => {
            ...
            const changedTargetFile = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file2.editedContent(), null, null);
            const tu2a = changedTargetFile.transUnitWithId(ID_UNTRANSLATED_DESCRIPTION);
        ...
        it('should copy a transunit to a specified position (#53)', () => {
            ...
            const existingElem = dom_utilities_1.DOMUtilities.getElementByTagNameAndId(doc, 'translation', ID_EXISTING);
            const newElem = dom_utilities_1.DOMUtilities.getElementByTagNameAndId(doc, 'translation', ID_TO_MERGE);
        ...
        it('should copy a transunit from file a to file b, but should not set a praefix and suffix for ICU messages', () => {
            const file = readUnknownFormatFile(MASTER_1_XMB);
            const tu = file.transUnitWithId(ID_ICU_SELECT_2);
            expect(tu).toBeTruthy();
            const targetFile = readFile(TRANSLATION_EN_XTB, MASTER_1_XMB);
            targetFile.setNewTransUnitTargetPraefix('%%');
            targetFile.setNewTransUnitTargetSuffix('!!');
        ...
        it('should translate using NormalizedMessage (plain text case, no placeholders, no markup)', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_MY_FIRST);
...
//# sourceMappingURL=xtb-file.spec.js.map