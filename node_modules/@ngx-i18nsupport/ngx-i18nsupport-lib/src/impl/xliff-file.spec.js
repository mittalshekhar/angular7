...
describe('ngx-i18nsupport-lib xliff 1.2 test spec', () => {
    ...
    function readFile(path) {
        ...
        return index_1.TranslationMessagesFileFactory.fromFileContent('xlf', content, path, ENCODING);
    }
    describe('xlf 1.2 format tests', () => {
        ...
        it('should read xlf file', () => {
            const file = readFile(MASTER1SRC);
            expect(file).toBeTruthy();
            expect(file.fileType()).toBe('XLIFF 1.2');
            const tu = file.transUnitWithId(ID_TRANSLATED_SCHLIESSEN);
        ...
        it('should not add empty lines when beautifying (issue ngx-i18nsupport #97)', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromFileContent('xlf', editedContentBeautified, null, ENCODING);
        ...
        it('should return target language', () => {
            ...
            const translatedFile = readFile(TRANSLATED_FILE_SRC);
            expect(translatedFile.targetLanguage()).toBe('de');
        });
        it('should change target language', () => {
            const file = readFile(TRANSLATED_FILE_SRC);
        ...
        it('should loop over all trans units', () => {
            const translatedFile = readFile(TRANSLATED_FILE_SRC);
            let count = 0;
            translatedFile.forEachTransUnit((tu) => {
                expect(tu).toBeTruthy();
                count++;
            });
        ...
        it('should change source content', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_BR_TAG);
            expect(tu).toBeTruthy();
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_WITH_BR_TAG);
            expect(tu2.sourceContent()).toBe(changedMessage);
        });
        it('should read meaning and description of tu', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_MEANING_AND_DESCRIPTION);
            expect(tu).toBeTruthy();
        ...
        it('should change description', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_WITH_MEANING_AND_DESCRIPTION);
            expect(tu2.description()).toBe(changedMessage);
        });
        it('should set description (creates new description)', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_BR_TAG); // anyone without description
            expect(tu).toBeTruthy();
            ...
            const tu2 = file2.transUnitWithId(ID_WITH_BR_TAG);
            expect(tu.description()).toBe(changedMessage);
            const xmlElem = tu2.asXmlElement();
            const noteElem = dom_utilities_1.DOMUtilities.getFirstElementByTagName(xmlElem, 'note');
        ...
        it('should read additional notes, issue #56', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_NOTES);
            expect(tu).toBeTruthy();
            const notes = tu.notes();
        ...
        it('should edit additional notes, issue #56', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_WITH_NOTES);
        ...
        it('should return empty source references array if source not set', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_NO_SOURCEREFS);
            expect(tu).toBeTruthy();
            expect(tu.sourceReferences().length).toBe(0);
        });
        it('should return source reference', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_ONE_SOURCEREF);
            expect(tu).toBeTruthy();
        ...
        it('should return more than one source references', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_TWO_SOURCEREFS);
            expect(tu).toBeTruthy();
        ...
        it('should set source references', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_TO_MERGE);
            expect(tu).toBeTruthy();
            expect(tu.sourceReferences().length).toBe(0);
            tu.setSourceReferences([{ sourcefile: 'x', linenumber: 10 }, { sourcefile: 'y', linenumber: 20 }]);
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_TO_MERGE);
        ...
        it('should override source references', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_WITH_TWO_SOURCEREFS);
        ...
        it('should run through 3 different states while translating', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_PLACEHOLDER);
            expect(tu).toBeTruthy();
            expect(tu.targetState()).toBe(index_1.STATE_NEW);
            tu.translate('a translation');
            expect(tu.targetState()).toBe(index_1.STATE_TRANSLATED);
            tu.setTargetState(index_1.STATE_FINAL);
            expect(tu.targetState()).toBe(index_1.STATE_FINAL);
        });
        it('should normalize placeholders to {{0}} etc', () => {
            ...
            const tu2 = file.transUnitWithId(ID_WITH_PLACEHOLDER_2);
        ...
        it('should normalize repeated placeholders to {{0}} {{1}} etc', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_REPEATED_PLACEHOLDER);
        ...
        it('should normalize empty html tag img', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_WITH_IMG_TAG);
        ...
        it('should translate source without or with target', () => {
            ...
            const file2 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file.editedContent(), null, null);
            const tu2 = file2.transUnitWithId(ID_TRANSLATED_SCHLIESSEN);
            expect(tu2.targetContentNormalized().asDisplayString()).toBe('Anwendung läuft');
            expect(tu2.targetState()).toBe(index_1.STATE_TRANSLATED);
            // translate again
            tu2.translate('Anwendung funktioniert');
            const file3 = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(file2.editedContent(), null, null);
            const tu3 = file3.transUnitWithId(ID_TRANSLATED_SCHLIESSEN);
            expect(tu3.targetContentNormalized().asDisplayString()).toBe('Anwendung funktioniert');
            expect(tu3.targetState()).toBe(index_1.STATE_TRANSLATED);
        });
        it('should copy source to target for default lang', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_UNTRANSLATED_DESCRIPTION);
            expect(tu).toBeTruthy();
            expect(tu.targetContent()).toBeFalsy();
            const isDefaultLang = true;
            const copyContent = false;
            const file2 = file.createTranslationFileForLang('xy', null, isDefaultLang, copyContent);
            const tu2 = file2.transUnitWithId(ID_UNTRANSLATED_DESCRIPTION);
        ...
        it('should copy source to target for non default lang and set state new, native state initial, #57', () => {
            ...
            const file2 = file.createTranslationFileForLang('xy', null, isDefaultLang, copyContent);
            const tu2 = file2.transUnitWithId(ID_UNTRANSLATED_DESCRIPTION);
            expect(tu2.targetState()).toBe(index_1.STATE_NEW);
        ...
        it('should put new target element directy behind source element, #50', () => {
            ...
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_NO_TARGET);
            expect(tu).toBeTruthy();
            expect(tu.targetContent()).toBeFalsy();
            const isDefaultLang = false;
            const copyContent = true;
            const file2 = file.createTranslationFileForLang('xy', null, isDefaultLang, copyContent);
            const tu2 = file2.transUnitWithId(ID_NO_TARGET);
            expect(tu2.targetContent()).toBeTruthy();
            // check the xml of file that target directly follows source
            const document = new xmldom_1.DOMParser().parseFromString(file2.editedContent(), 'text/xml');
            const targetList = document.getElementsByTagName('target');
            for (let i = 0; i < targetList.length; i++) {
                const targetNode = targetList.item(i);
                const sibling = dom_utilities_1.DOMUtilities.getPreviousElementSibling(targetNode);
        ...
        it('should copy a transunit from file a to file b', () => {
            ...
            const tu = file.transUnitWithId(ID_TO_MERGE);
            expect(tu).toBeTruthy();
            const targetFile = readFile(TRANSLATED_FILE_SRC);
            ...
            const changedTargetFile = index_1.TranslationMessagesFileFactory.fromUnknownFormatFileContent(targetFile.editedContent(), null, null);
        ...
        it('should copy a transunit to a specified position (#53)', () => {
            ...
            expect(targetFile.transUnitWithId(ID_TO_MERGE)).toBeTruthy();
            expect(targetFile.transUnitWithId(ID_TO_MERGE)).toEqual(newTu);
            const doc = new xmldom_1.DOMParser().parseFromString(targetFile.editedContent());
            const existingElem = dom_utilities_1.DOMUtilities.getElementByTagNameAndId(doc, 'trans-unit', ID_EXISTING);
            const newElem = dom_utilities_1.DOMUtilities.getElementByTagNameAndId(doc, 'trans-unit', ID_TO_MERGE);
        ...
        it('should copy source to target, but should not set a praefix and suffix for ICU messages', () => {
            ...
            const copyContent = true;
            const file2 = file.createTranslationFileForLang('xy', null, isDefaultLang, copyContent);
            const tuICU = file2.transUnitWithId(ID_ICU_PLURAL);
            expect(tuICU.targetState()).toBe(index_1.STATE_NEW);
        ...
        it('should copy a transunit from file a to file b, but should not set a praefix and suffix for ICU messages', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_ICU_SELECT);
            expect(tu).toBeTruthy();
            const targetFile = readFile(TRANSLATED_FILE_SRC);
            targetFile.setNewTransUnitTargetPraefix('%%');
            targetFile.setNewTransUnitTargetSuffix('!!');
        ...
        it('should translate using NormalizedMessage (plain text case, no placeholders, no markup)', () => {
            ...
            // first translate
            const translation = tu.sourceContentNormalized().translate(translationString);
            tu.translate(translation);
        ...
        it('should contain ICU reference in sourceContentNormalized', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_CONTAINS_ICU);
            expect(tu).toBeTruthy();
        ...
        it('should contain 2 ICU references in sourceContentNormalized', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_CONTAINS_TWO_ICU);
            expect(tu).toBeTruthy();
        ...
        it('should handle plural ICU', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_ICU_PLURAL);
            expect(tu).toBeTruthy();
        ...
        it('should handle ICU with embedded tags', () => {
            const file = readFile(MASTER1SRC);
            const tu = file.transUnitWithId(ID_ICU_EMBEDDED_TAGS);
            const normalizedMessage = tu.sourceContentNormalized();
            expect(normalizedMessage.asDisplayString()).toBe('<ICU-Message/>');
            const icuMessage = normalizedMessage.getICUMessage();
            expect(icuMessage).toBeTruthy();
...
//# sourceMappingURL=xliff-file.spec.js.map