...
describe('normalized message test spec', () => {
    ...
    function parsedMessageFor(normalizedString, sourceMessage) {
        const parser = new xliff2_message_parser_1.Xliff2MessageParser(); // parser does not matter here, every format should be the same.
     ...
     */
    function parsedICUMessage(icuMessageString, sourceMessage, parserType) {
        let parser;
        if (sourceMessage) {
            ...
            if (parserType) {
                switch (parserType) {
                    case 'xlf':
                        parser = new xliff_message_parser_1.XliffMessageParser();
                        break;
                    case 'xlf':
                        parser = new xmb_message_parser_1.XmbMessageParser();
                        break;
                    case 'xlf2':
                        parser = new xliff2_message_parser_1.Xliff2MessageParser();
                        break;
                }
            }
            else {
                parser = new xliff2_message_parser_1.Xliff2MessageParser(); // parser does not matter here, every format should be the same.
            }
        }
    ...
    describe('ICU test cases', () => {
        ...
        it('should parse plural ICU message with placeholder in xlf format', () => {
            ...
            const parsedMessage = parsedICUMessage(original, null, 'xlf');
        ...
        it('should parse plural ICU message with placeholder in xlf2 format', () => {
            ...
            const parsedMessage = parsedICUMessage(original);
            expect(parsedMessage.asDisplayString()).toBe('<ICU-Message/>');
            expect(parsedMessage.getICUMessage()).toBeTruthy();
            const icuMessage = parsedMessage.getICUMessage();
            expect(icuMessage.getCategories().length).toBe(3);
...
//# sourceMappingURL=parsed-message.spec.js.map