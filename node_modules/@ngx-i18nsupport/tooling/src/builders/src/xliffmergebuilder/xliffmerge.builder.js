...
class XliffmergeBuilder {
    ...
    createProgramOptionsFromConfig(builderConfig) {
        ...
        if (profile) {
            ...
            const profilePath = `${core_1.getSystemPath(root)}/${profile}`;
    ...
    createProfileContentFromConfig(builderConfig) {
        const xliffmergeOptions = builderConfig.options.xliffmergeOptions;
        if (xliffmergeOptions) {
            const wsRoot = this.context.workspace.root;
